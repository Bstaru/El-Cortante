...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `i_noticia_cImg`(
...
BEGIN
		...
		SET @lastID_noticia = LAST_INSERT_ID();
        
        INSERT INTO media(path,idNoti)
		VALUES (img,@lastID_noticia);
	END$$
DELIMITER ;
...
BEGIN
SELECT idUsuario,nombreU,AP,AM,idNoti,titulo,descrip,activo,imgN FROM v_busqueda
